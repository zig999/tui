...
description: Cross-session persistent log for escalation resolutions, architectural decisions, and implementation patterns made by the backend orchestrator and developer agents. Prevents repeated mistakes across sessions.
user-invocable: false
---

# Session Decisions Log (Backend)
...
| `Type` | `architectural` \| `spec_gap` \| `triage_resolution` \| `escalation` \| `qa_root_cause` | Classification |
...
| `Status` | `active` \| `superseded` \| `resolved` | Current validity |
...
- Architectural decision with project-wide impact (type: `architectural`)
- Spec gap confirmed during implementation (type: `spec_gap`)
- Triage resolution that closes a validation error (type: `triage_resolution`)
- Agent escalation to human (type: `escalation`)
- QA root-cause finding that reveals a systemic pattern (type: `qa_root_cause`)
